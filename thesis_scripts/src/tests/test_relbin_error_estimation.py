...
def test_relbin_error_estimation():
    ...
    n_f_relbin = 20
    n_freqs_relbin = np.geomspace(1e2, 1e5, num=n_f_relbin, dtype=int)
    like = LunarLikelihood()
    like.compute_center(injection_params['time_at_center'])
    ...
    for j, n_freq in enumerate(n_freqs_relbin):
        ...
        for i, idx in tqdm(enumerate(random_indices)):
            ...
            estimated_errors_relbin[i, j] = like.relbin_log_likelihood_error(params)
