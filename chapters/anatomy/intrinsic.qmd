...
Due to precession, the orbital angular momentum $\vec{L}$ and the spins $\vec{S}$ are time-dependent. The total angular momentum $\vec{J}$, on the other hand, is conserved, and so is its angle with the observation direction $\theta_{\text{JN}}$.

Also see @romero-shawBayesianInferenceCompact2020

## Eccentricity

Generic Keplerian orbits are eccentric, describing an ellipse with the center of mass as a focus. 
...
Most of the gravitational waves detected so far have had little to no eccentricity in the 

```{python}
#| label: fig-gw250114-eccentricity-posterior
#| fig-cap: "Eccentricity posterior distribution for GW250114."
...
# KDE the data together with a mirror with negative values
# in order to correctly model the boundary
kde = gaussian_kde(
    np.concatenate((ecc, -ecc)),
    bw_method=.25
)

e = np.linspace(0, 0.05)
plt.plot(e, 2*kde(e), c='blue')
plt.hist(ecc, density=True, histtype='step', bins=20, color='blue')
plt.xlabel('Eccentricity $e$')
plt.xlabel('Posterior distribution $p(e)$')
plt.show()
```
...
freqs, lo, med, hi = peters_evolution.get_gw250114_eccentricity_extrapolation()
