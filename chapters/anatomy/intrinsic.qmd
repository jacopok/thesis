...
In this chapter we continue in the dissection of the gravitational signal 
emitted by the coalescence of two compact objects by considering 
its intrinsic parameters, 
those relating to the source itself and not to its relation to the observer.

This gravitational signal is often qualitatively decomposed
into three sections: inspiral, merger and post-merger.

When the remnant is a black hole the post-merger is an exponentially damped ringdown, as described in @sec-ringdown; 
if this is not the case, the emission spectrum can become significantly less predictable and long-lived. 
Though efforts have been made toward modelling it---see e.g. [@breschiKiloHertzGravitationalWaves2019;@breschiKilohertzGravitationalWaves2022;@breschiKilohertzGravitationalWaves2024] for neutron star binaries---we will not discuss this in detail here.

The rest of this chapter mainly concerns the dependence of models for the inspiral on their parameters. 
Some of the ones we will consider are really only suitable for the early stages of inspiral, while others are accurate enough to describe the merger, and they can be smoothly connected to a model for the post-merger. 
When applied to black hole binaries, the result is an Inspiral-Merger-Ringdown (IMR) waveform model.

This chapter also contains a discussion of the results we presented in @tissinoCombiningEffectiveonebodyAccuracy2023,
regarding the construction and application of a fast frequency-domain surrogate for neutron star binaries,
as well as the ones in @gambaRevisitingGW150914Nonplanar2025, regarding
a re-analysis of the GW150914 event with a waveform model accounting for both 
eccentricity and spin-precession.
...
A general Post-Newtonian expression for the phase of a compact binary reads as follows [@abacScienceEinsteinTelescope2026, eq. 1.1]:
$$
...
If chirp mass were the only parameter allowed to vary, the frequency derivative of the signal $\dot{f}$, or equivalently in the stationary phase approximation (SPA, see @sec-stationary-phase-approximation) the second derivative of the frequency-domain phase $\Psi''$ in @eq-post-newtonian-phase-generic, would be sufficient to constrain it.
...
When sampling, it is also important to identify which object is the primary and which is the
...
Qualitatively, $\chi_{\text{eff}}$ was slighty negative and $\chi_p$ was small.
...
Note how the scale of the plot for both components could in principle be extended to a magnitude of 1.
...
If the prior on the spin magnitude $\pi(|\chi|)$ is taken to be uniform in the range $[0, \chi_{\text{max}}]$, then the result of this integral is [@langeRapidAccurateParameter2018, eq. A7]:

$$p(\chi_{z}) = - \ln ( |\chi_{z} / \chi^{\text{max}}|) / 2 \chi^{\text{max}}\,.
$${#eq-aligned-spin-prior-uniform-in-magnitude}
...
The constants of motion $p$ and $e$ are called _semilatus rectum_ and _eccentricity_. 
...
orbits are also not exactly elliptical, due to both perihelion precession and gravitational wave emission [@favataGravitationalwaveMemoryEccentric2011].
...
This frequency is monotonic when the orbit is quasi-circular, while for eccentric orbits it exhibits peaks and troughs on the orbital timescale [@shaikhDefiningEccentricityGravitational2023].
...
It would be possible to also account for the impact of spin-precession, as described by @fumagalliResidualEccentricitySystematic2024.
...
axs[0].set_title('Uniform prior on $e_{{13.33\mathrm{{Hz}}}}$')
...
axs[1].legend()
axs[1].set_title(r'Log-uniform prior on $e_{{13.33\mathrm{{Hz}}}}$')

plt.show()
```

While the limits and analysis quoted in the discovery paper [@abacGW250114TestingHawkings2025] used a uniform prior in $e_{13.33\text{Hz}}$, 
a conservative choice, we also show results obtained with a log-uniform prior $p(e_{13.33\text{Hz}}) \propto 1/e_{13.33\text{Hz}}$.
...
For an in-depth discussion of this effect, as well as an exploration of how one might use astrophysically-motivated priors instead, see @clarkeUniversalFrameworkIdentify2026.

## Tidal parameters {#sec-tidal-parameters}

Objects such as neutron stars can be deformed by the tidal gravitational field 
caused by the presence of a nearby object, such as the binary companion. 
...
In the static case, this tidal field can be expressed as $\mathcal{E}_{ij}=C_{0i0j}$, 
where $C$ is the Weyl curvature tensor [@misnerGravitation1973, eq. 13.50], 
the trace-free component of the Riemann curvature tensor. At leading order, 
this tidal field will induce a quadrupole moment $Q_{ij}$, given by 
...
where $R$ is the radius of the object, while $\kappa_{2}$ is its $\ell=2$ dimensionless 
tidal Love number, which for neutron stars is generally on the order of 0.1.

The main tidal contribution to the waveform is through the effective tidal parameter, 
which enters the expansion at 5PN order [@wadeSystematicStatisticalErrors2014]:
$$
...
\end{aligned}
$$
While their expressions are intimidating, note that several terms vanish in the equal-mass case, 
where $\nu = 1 / 4$; if that is true then $\tilde{\Lambda} =f(\nu) (\Lambda_{1}+\Lambda_{2})$ 
and $\delta \tilde{\Lambda} = g(\nu)(\Lambda_{1}-\Lambda_{2})$ for some functions $f$ and $g$. 
Furthermore, if the masses are equal and so are the tidal parameters ($\Lambda_{1}=\Lambda_{2}=\Lambda$), 
then we have $\Lambda = \tilde{\Lambda}$ and $\delta \tilde{\Lambda} =0$.

The analysis so far has focused on the adiabatic, quadrupolar case:
assuming that only the dominant oscillation mode of a neutron star
is excited, and that its time evolution
follows the forcing term (_i.e._ the other star's tidal field). 

In reality, the tidal deformation of a neutron star is more accurately described 
through a decomposition including the time-dependent excitation of different modes.
There are several modes of oscillation one might consider, which have been deeply
discussed in the field of astroseismology, 
[@cunhaAsteroseismologyInterferometry2007;@steinhoffSpinEffectsNeutron2021;@zhaoUniversalRelationsNeutron2022], 
and a full discussion of them is beyond the scope of this work. 
The amplitude $Q_n$ of each of these modes, which we index with an integer $n$, will oscillate 
following a driven harmonic oscillator [@laiResonantOscillationsTidal1994] in the form

$$\ddot{Q}_n + \frac{\dot{Q}_n}{t_{\text{diss}}} + \omega_n^2 Q^2 = f_n (Q_n, t, \text{orbit})\,,
$$

for some values of the dissipative timescale $t_{\text{diss}}$ and angular frequency 
$\omega_n$. 
The forcing term $f_n$ will generally depend on the relative position of the companion star.
In the quasi-circular case this can be approximated as a sinusoid 
whose period scales inversely with the angular frequency of the binary
$\omega_{\text{orb}}$, but for eccentric binaries high-frequency modes may be excited 
even though the orbit-averaged angular frequency is comparatively low.

The adiabatic assumption is equivalent to the requirement that 
$1/t_{\text{diss}} \ll \bar{\omega}_{\text{orb}}$, 
as well as $\omega_n \gg \bar{\omega}_{\text{orb}}$
[@flanaganConstrainingNeutronStar2008]. 
The first constraint is generally expected to be well-satisfied, 
both when discussing losses dueto gravitational wave emission 
[@takatsyObservabilityDynamicalTides2024] as well as 
due to friction [@ripleyConstraintDissipativeTidal2024].

Relaxing the $\omega_n \gg \bar{\omega}_{\text{orb}}$ assumption means considering 
the effect of _dynamical_ (also known as _resonant_) tides.
These can impact the phasing at low frequencies, especially in the case of eccentric binaries
[@gambaResonantTidesBinary2023].
Considering oscillations with angular structure at higher orders than the quadrupole is also possible
[@jimenez-fortezaImpactHighorderTidal2018] 
In any case, none of these higher-order corrections are required to explain current 
observations of neutron star mergers: the clearest to date, GW170817,
only provided comparatively weak constraints on the leading-order tidal parameters $\Lambda$
[@theligoscientificcollaborationPropertiesBinaryNeutron2019].
...
There are several approaches to modelling the waveform emitted by a compact binary inspiral, 
with varying degrees of computational complexity, 
completeness in the inclusion of physical effects,
and accuracy. For example, while all the approaches discussed here 
can in principle include any amount of modes 
in the spherical harmonic decomposition of binary emission (see @sec-phase-inclination-angle),
some specific models only include the dominant one.
...
We will focus on the Post-Newtonian (@sec-post-newtonian) and Effective One Body (@sec-eob) approaches to waveform generation; we also note the existence of Numerical Relativity surrogates (_e.g._ `NRSur7dq4`, presented by @varmaSurrogateModelsPrecessing2019) and phenomenological models (_e.g._ `IMRPHenomXHM`, presented by @garcia-quirosIMRPhenomXHMMultimodeFrequencydomain2020) for comparable mass binaries, and the gravitational self-force formalism for extreme mass ratios (_e.g._ the second-order self-force calculations presented by  @poundSecondOrderSelfForceCalculation2020 and implemented in the Fast EMRI Waveforms software package [@katzFastEMRIWaveformsNewTools2021]).

### Post-Newtonian {#sec-post-newtonian}

Post-Newtonian waveforms are obtained by analytically 
expanding Einstein's equations in powers of $v/c$. 
Thanks to the stationary phase approximation, this can lead 
to an analytical expression for the amplitude and phase as a function of frequency. 

These models are generally only accurate for a description of the early inspiral.

@buonannoComparisonPostNewtonianTemplates2009 give a comparison 
of some commonly used Post-Newtonian waveform approximants, including the frequency-domain one `TaylorF2`, 
whose frequency-domain waveforms read [@messinaQuasi55PNTaylorF2Approximant2019]:
$$
...
Effective One Body waveforms are obtained by mapping the relativistic 
two body problem to the motion of a test particle in a deformed metric. 
The following description will mainly describe 
the components of the `TEOBResumS` family of models, 
though similar considerations also apply to other models, 
such as those in the `SEOB` family.
...
$$
where the effective Hamiltonian contains a plethora of physical effects and Post-Newtonian contributions. 
...
a surrogate of the frequency-domain inspiral-only model `TEOBResumSPA` [@gambaFastFaithfulFrequencydomain2020] for aligned-spin neutron star binaries
with tidal effects --- a 5-dimensional parameter space, with parameters $(q, \chi_1, \chi_2, \Lambda_1, \Lambda_2)$, since all other parameters can be modeled analytically.
...
![Evaluation time for `mlgw_bns` compared to its reference model `TEOBResumSPA` and some others. For every model, we also show a fit of the waveform evaluation time $t$ with a linear function of the number of frequency grid points $N$, so that $t = a + bN$. ](figures/mlgw_bns/fig12.pdf){#fig-mlgw-bns-compared-timing width=60%}
