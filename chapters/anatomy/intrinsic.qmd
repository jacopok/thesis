...
# Intrinsic parameters and waveform modelling {#sec-intrinsic}
...
The motion of either body can easily be recovered from this equation: their radii as measured from the center of mass will always have opposite signs, and magnitudes $r_{1} = q r / (1+q)$ and $r_{2} = r / (1+q)$, where $q$ is the mass ratio. 
...
The *eccentricity* $e$ is given by $e^2 = 1- ( b / a)^2$.
...
It is possible to define a notion of eccentricity based on the evolution of the emission frequency of the binary, which is monotonic when the orbit is quasi-circular, while it exhibits peaks and trophs on the orbital timescale for eccentric orbits [@shaikhDefiningEccentricityGravitational2023].

Within the `TEOBResumS-DAlí` EOB waveform model [@boninoInferringEccentricityEvolution2022], 
...
@gambaRevisitingGW150914Nonplanar2025 performed a first-of-its-kind reanalysis of the first-ever detection GW150914 with a model including both eccentricity and precessing spins. 
...
![Eccentricity and true anomaly posterior for GW150914, from @gambaRevisitingGW150914Nonplanar2025.](figures/ecc_zeta.pdf){#fig-ecc-zeta-gw150914}
...
to be necessary for the data currently available, 
constraining $\tilde{\Lambda}$ to be less than 
155 (with uniform priors) or 35 (with log-uniform priors).
...
and accuracy.
Beyond the ones listed here, we also note the existence of __Numerical Relativity surrogates__ and __phenomenological models__.
...
The surrogate model should also be faster to evaluate: this was indeed the case, as @fig-mlgw-bns-compared-timing shows.
...
![Evaluation time for `mlgw_bns` compared to its reference model `TEOBResumSPA` and some others. ](figures/mlgw_bns/fig12.pdf){#fig-mlgw-bns-compared-timing}
