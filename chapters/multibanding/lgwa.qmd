...
reweight the samples obtained in zero noise with 1000 realizatons of the noisy likelihood.
...
We start from $2.6 \times 10^4$ posterior samples, and as @fig-sampling-efficiencis-noisy shows, are sometimes left with less than a hundred effective samples after the reweighting procedure.

However, as that figure also show, there is an alternative: we can perform the reweighting starting from the full set of $1.7 \times 10^5$ nested samples.
...
In @fig-boxplot-noisy-reweight we show the r
...
![Waveforms for the 11 and 12 month normalized injections, shown in characteristic strain, together with the LGWA PSD and the artificial PSD used in the "uniform SNR" scenario.](figures/lgwa_waveforms_month_by_month.pdf){#fig-waveforms-lgwa-injections width=80%}
