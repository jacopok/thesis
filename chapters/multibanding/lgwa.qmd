...
For the purpose of validating our intuition, we want to construct a PSD such that the accumulation of SNR over time is uniform, _i.e._ the SNR obtained in any given month is equal to that obtained in another.  
The $22$ mode of the signal has an amplitude scaling with $f^{-7/6}$;
projecting the signal introduces month-long modulations, which we average over by only looking at integer numbers of months. 
...
$$
where we can obtain the frequency derivative by differentiating $t(f) \propto f^{-8/3}$. 
