...
![Convex hulls for the 12 month normalized injections.](figures/trajectories.png){#fig-trajectories-lgwa-injections width=80%}
...
The root-mean-square of $w(t)$ is related to the total SNR, 
...
In all cases, the Fisher matrix approximation overestimates the sky sensitivity of our detector; however, it should be stressed that even when accounting for this effect, the LGWA alone can still achieve better sky localization than pairs of ground-based interferometers, despite lower SNRs.
...
The strategy we propose is to perform the first parameter estimation
with a heuristically-determined origin, 
compute the optimal origin, which should always be used thereafter.
For the tests in this section, the heuristic we employed was to 
use the position of the detector