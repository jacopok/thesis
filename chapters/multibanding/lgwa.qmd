...
We need a frame that is _comoving_ with the SSB, but not necessarily _centered_ on the Sun, and this is an important distinction for the LGWA.
...
plt.semilogx(f, np.unwrap(np.angle(hx/hx2)), label='Center at SSB')
...
plt.semilogx(f, np.unwrap(np.angle(hx2/hx)), label="Center close to the Moon's position at merger")
plt.legend()
...
### Shifting reference frame in post-processing
...
![](figures/time_std_solar_system.pdf){#fig-timing-lgwa-solar-system width=80%}

![](figures/minimum_uncertainty_velocity.pdf){#fig-minimum-uncertainty-velocity width=80%}

![](figures/timing_constraint_by_frequency_GW250114.pdf){#fig-timing-lgwa-by-frequency width=80%}

### Spanned area scaling validation

![](figures/trajectories.pdf){#fig-trajectories-lgwa-injections width=80%}

![](figures/lgwa_waveforms_month_by_month.pdf){#fig-waveforms-lgwa-injections width=80%}

![](figures/sky_locs.pdf){#fig-injections-sky-areas width=80%}
