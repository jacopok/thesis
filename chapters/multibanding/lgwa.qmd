...
formed independently, are likely to have different isotopic compositions [@meloshNewApproachesMoons2014].
...
![Cumulative SNR for the population of compact binaries in the GWTC catalogs. The detection rate for the LGWA at any given SNR is comparable to that obtained by the LVK. From @iacovelliGravitationalwaveParameterEstimation2025.](figures/gw231123/cumulative_detections_wErrorbars_LGWA_GB.pdf){#fig-gwtc-snr-lgwa width=70%}
...
- GW170871, the best-constrained neutron star binary detected with gravitational waves, and the only multi-messenger GW detection to date [@abbottGW170817ObservationGravitational2017;@theligoscientificcollaborationPropertiesBinaryNeutron2019];
...
For more details, see @tissinoGeometryLunarGravitational2026 and its attached data release [@tissinoGeometryLunarGravitational2026a].

```{python}
#| label: tbl-priors-full-analysis-gw170817
#| tbl-cap: Priors and injected values for our GW170817-like injection. The notation is the same as in @tbl-priors-full-analysis.
...
| $\chi_1$                                       |        0.0245664 |           | Uniform                     | [-0.8, 0.8]                                                  |
| $\chi_2$                                       |       -0.0100164 |           | Uniform                     | [-0.8, 0.8]                                                  |
| $\mathrm{DEC}$                                 |       -0.571666  | rad       | Cosine                      | $[-0.01, 0.01] + \mathrm{dec}_{\mathrm{inj}}$                |
| $\lambda_1$                                    |      413.54      |           | Uniform                     | [0, 5000]                                                    |
| $\lambda_2$                                    |      369.35      |           | Uniform                     | [0, 5000]                                                    |
| $d_L$                                          |       37.2307    | Mpc       | Uniform in the source frame | [5, 100]                                                     |
| $q$                                            |        0.762902  |           | Uniform                     | [0.5, 1]                                                     |
| $\phi$                                         |        3.19071   |           | Uniform                     | [0, 2$\pi$]                                                  |
| $\psi$                                         |        0.43413   | rad       | Uniform                     | [0, $\pi$]                                                   |
| $\mathrm{RA}$                                  |        6.13476   | rad       | Uniform                     | $[-0.01, 0.01] + \mathrm{ra}_{\mathrm{inj}}$                 |
| $\theta_{JN}$                                  |        2.43295   | rad       | Sine                        | [0, $\pi$]                                                   |
...
of the smooth Gaussian process envelopes used for the inference of interferometric data would 
...
### Impact of non-zero Gaussian noise
...
to be _representative_ of the distributions $p(\theta | d = h_0 + n)$ in some sense.
In this section we investigate the impact of non-zero Gaussian noise realizations on our GW250114 injection.

In order to to so, we need to explore the space of distributions $p(\theta | d = h_0 + n)$ with varying noise $n \sim \mathcal{L}(n)$,
...
use a phase-marginalized likelihood, and only sample over a reduced parameter space, including 
...
and it allows us to use the full parameter space, but it comes at the expense of a potential bias. 
...
These injections are closer to a realistic scenario than the zero-noise case, but still not fully realistic: they are performed assuming that the noise is Gaussian, and colored according to a known power spectral density; neither of these will be true in practice.
...
|---:|----:|---:|
| global optimum  | $f_{22}=0.56$ Hz | 0.09 s |
| Earth | $f_{22}=0.56$ Hz | 0.15 s |
| Moon | merger | 0.24 s |
| Earth | merger | 0.24 s |
| Solar System Barycenter | merger | 11.16 s |
| Solar System Barycenter | $f_{22}=0.56$ Hz | 11.25 s |
| Earth | $f_{22}=0.03$ Hz | 225 s |
""")
```
...
![Spatial variation of the timing uncertainty for our GW250114-like injection. ](figures/time_std_solar_system.pdf){#fig-timing-lgwa-solar-system width=80%}
...
Fisher matrix results (@eq-wen-chen-area) suggest that the sky localization obtained with a long observation --- all else being equal --- scales with the inverse of the area spanned by the detector's motion, projected onto the plane orthogonal to the source's propagation direction.
...
![Waveforms for the 11 and 12 month normalized injections, shown in characteristic strain, together with the LGWA PSD and the artificial PSD used in the "uniform SNR" scenario.](figures/lgwa_waveforms_month_by_month.pdf){#fig-waveforms-lgwa-injections width=70%}
...
![Top panel: convex hulls for the 12 month normalized injections, projected onto a plane orthogonal to the gravitational wave propagation direction. The line thickness is proportional to the SNR integrand as a function of time, defined in @nte-snr-integrand. Bottom panel: SNR integrand as a function of time on a linear axis.](figures/trajectories.png){#fig-trajectories-lgwa-injections width=90%}

![Scaling of the geometric and statistical areas.](figures/areas_appendix.pdf){#fig-geometric-statistical-areas width=70%}
...
$$ \text{SNR}^{2} = \int \underbrace{ \frac{|h(f)|^{2}}{S_{n}(f)} \frac{\text{d}f}{\text{d}t} }_{ w^2(t) } \text{d}t\,.
$$

The root-mean-square of $w(t)$, taken as a function of time, is proportional to the total SNR.
...
![Comparison between the Fisher matrix expectation for the sky localization and the injections performed in the three scenarios described in @sec-area-validation.](figures/sky_loc_vs_theory.pdf){#fig-injections-vs-wen-chen width=80%}

As a general pattern, we see how the qualitative Fisher matrix prediction is validated for all but the 12 month case: the sky localization area decreases with the number of months, even though the SNR is kept constant, driven mostly by the change in area; the frequency increases slightly through the injections, but not nearly enough to explain the effect.
...
We validate this intuition through the __simplified, uniform SNR__ scenario, in which the SNR is accumulated at a constant rate over time --- we obtain this by artificially setting a PSD in the form $S_{n}(f) \propto f^{4/3}$, as shown in @fig-waveforms-lgwa-injections. 
...
The Fisher matrix approach also predicts a larger sky area for the 1-month injection than what we observe in the simplified scenario. 
We attribute this to the fact that it assumes a constant antenna pattern, 
whereas during a month of observation the Moon's rotation imprints a measurable, sky position-dependent amplitude modulation to the signal. 
This effect is present in all cases, but the precision of the Doppler shift-driven sky localization dominates once the observation lasts more than a month.
...
This is because the convex hull of the observation is not significantly affected if a small fraction of the perimeter is removed.
...
As we shall shortly show, this entails a significant computational multiplier:
doing so for the complete injection, which requires a good amount of resources
even with a close-to-optimal frame would be wasteful and lengthy. 
