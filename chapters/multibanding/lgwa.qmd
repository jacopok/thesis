...
though the chance a gravitational wave signal strong enough to be is detected by it comes is slim.
...
The scalar lunar response $L$ is a function of frequency with units of length, and it is defined as the ratio between the amplitude of the surface displacement $s$ and the amplitude of the gravitational wave strain $h$ which drives it, assuming that both are monochromatic with a frequency $f_{0}$:
$$
L(f_{0}) = \frac{s(t;f_{0})}{h(t; f_{0})}\,,
$$
where the amplitude of the gravitational wave is obtained by contracting the gravitational wave strain tensor $h_{ij}$ with a geometric detector response $\hat{D}_{ij}$.
...
does not only include gravitational wave science [@ajithLunarGravitationalwaveAntenna2024].
...
Furthermore, the LGWA would be able to precisely measure the excitation of lunar normal modes by Moonquakes.^[As discussed earlier, their excitation by gravitational waves would require an improbably strong signal.] This would provide insights into the lunar interior structure, enabling the exploration of lunar origin models.
The formation mechanism of the Moon is not a settled debate: the giant impact scenario, in which a large planetary object dubbed _Theia_ impacted a young Earth thus forming a disk of debris which coalesced into our satellite, is the dominant theory, but it does not stand unchallenged. 
The _isotopic crisis_ is the problem posed by the observation that the Earth and Moon's mantles have nearly identical ratios of isotopes such as $^{17}O / ^{16}O$, $^{50}Ti / ^{47}Ti$ and $^{182}W / ^{184}W$. This is unlike any other body in the Solar System, for which these ratios are significantly different, and is hard to explain under the giant impact scenario. 
Mapping the composition of the lunar interior, which can be accomplished through seismic observations, will aid in the resolution of this problem.
...
![Time to merger as a function of mass, highlighting at which point various sources would cross the decihertz band. The sources are assumed to have equal mass and no spin - including these effects changes the results, but not the overall qualitative picture.](figures/time_to_merger.pdf){#fig-time-to-merger-lgwa width=80%}
...
We depict GW170817, the lightest binary whose merger has been detected to date, and GW190521, among the heaviest.
These are at the edges of what are known as _stellar-mass binaries_, with component masses roughly between $1$ and $100 M_{\odot}$, whose population has been explored by ground-based observations.
As illustrated in @fig-time-to-merger-lgwa, depending on their mass, they may cross the deci-Hertz band hours to years before their merger. 

![The population of compact binaries in the GWTC-3 catalog, highlighting GW231123, compared to the noise curves of LGWA and ground-based detectors. From @iacovelliGravitationalwaveParameterEstimation2025.](figures/gw231123/GW231123-ASD.pdf){#fig-231123-asd-lgwa width=80%}

In @fig-231123-asd-lgwa we further illustrate this aspect: a very massive binary like GW231123 would have been detectable by the LGWA, accumulating most of its SNR in the last day before merger.
This figure is meant to give an order of magnitude comparison between detector noise and signal magnitude, but since it does so with several different detectors and binaries it cannot be an apples-to-apples comparison: we should compare _projected_ strain at the detector to the detector's noise, but this would mean showing several different strain curves for every signal, as the projection onto each detector will be different. 
...
In @fig-gwtc-snr-lgwa we compute several realizations of a population compatible with the GWTC catalog, and compute their SNRs with various detectors, compared with the observed SNRs of the actual catalog.
The SNRs obtained with the LGWA are indeed compatible, within catalog variance, with the original ones.

![Cumulative SNR for the population of compact binaries in the GWTC catalogs. From @iacovelliGravitationalwaveParameterEstimation2025.](figures/gw231123/cumulative_detections_wErrorbars_LGWA_GB.pdf){#fig-gwtc-snr-lgwa width=80%}
...
![Horizon for equal mass black hole binaries as seen by the detectors for the next decade: Einstein Telescope, Lunar Gravitational Wave Antenna and LISA. From @ajithLunarGravitationalwaveAntenna2025.](figures/horizon_IMBH.pdf){#fig-horizon-multiband width=80%}
...
At these masses, the signals' mergers will be in band, and the signals may well show signs of eccentricity or precession; several higher order modes will be necessary for a complete description of the signal. 
...
So far we discussed comparable-mass binaries, but Intermediate Mass Ratio Inspirals, binaries with mass ratios in the range $q=10^{-2}$ to $10^{-4}$, would also be a fascinating way to shed light into the formation of intermediate-mass black holes.
...
Mergers of white dwarfs (WDs) are an interesting event, as they may be the origin of certain supernovae. They generally happen when the binary is emitting GWs in the deciHertz band.
Their collision is not preceded by a measurable "chirp", as the binary is still quite wide, but it generates a nuclear detonation or deflagration.
Collisions of WDs with other compact objects, such as neutron stars, are also of interest, also because they would emit at higher frequencies, which places them closer to the best sensitivity of LGWA around 0.3Hz, whereas double white dwarf systems generally collide before this point.
The main limitation in the study of their systems is their low rate: they are comparatively rare in the local Universe, the gravitational wave amplitudes they emit are not very high, meaning that they would need to be quite close - on the scale of a Megaparsec or less - to be detectable.

Tidal disruption events also can involve white dwarfs, for example a WD being disrupted after orbiting an intermediate mass black hole would produce a short-lived signal peaking in the deci-Hertz band.

Core-collapse supernovae may also exhibit significant emission in the deci-Hertz band, but current simulations typically last for a few seconds, and are therefore unable to resolve this part of the spectrum. 

Finally, the LGWA could contribute to the detection of a stochastic cosmological background of gravitational waves, but it would give significant results only by correlating the measurements of stations with a large separation, for example located at either lunar pole.
...
These injections are performed in zero noise, meaning that we investigate a Whittle likelihood (@sec-whittle-likelihood) in the form
$$
\log \mathcal{L}(\theta) = \mathrm{Re} (h(\theta)|h(\theta_{0})) - \frac{1}{2} (h(\theta)|h(\theta)) \,,
$$
...
We inject parameters compatible with two of the most notorious signals detected by the LVK collaboration:

- GW170871, the best-constrained neutron star binary detected with gravitational waves, and the only multi-messenger detection to date [@abbottGW170817ObservationGravitational2017;@theligoscientificcollaborationPropertiesBinaryNeutron2019];
- GW250114, the best-constrained black hole binary to date, which we discussed in detail in @sec-gw250114 [@abacGW250114TestingHawkings2025].
...
For GW250114 we could have chosen a longer observation, but as most of the SNR is accumulated in the last year before merger, we do not expect this would have led to significantly different results; truncating the observation this way highlights the fact that the constraints we see only require one year to be established.

The total SNR for GW170817 as seen by LGWA is 21.9, that of GW250114 is 34.8.

#### Limitations {#sec-lgwa-pe-limitations}

The procedure we employ regarding the response is equivalent to performing inference assuming that 
...
of the smooth Gaussian process envelopes used there would 
...
The zero-noise assumption is not expected to be a significant limitation. 
Adding a noise realization would simply bias the expectation value of our parameter estimation results, but not their variance, which is the quantity of interest in forecasting.
...
As we transition from forecasting to developing the analysis infrastructure for the mission, these issues can be addressed, eventually leading to a realistic simulation and blind analysis of the time-domain seismometer data.

#### Sky localization

Based on analytical results [@eq-wen-chen-area], we expect these long observations to tightly constrain the sky position of the sources, due to the significant Doppler modulation of the signal through the year being heavily dependent on it.

In the GW170817 case this is indeed the case, as shown in @fig-lgwa-gw170817-sky: the posterior is extremely tight, and it remains so when repeating the same experiment at the upper limit in frequency of $710\text{mHz}$, which corresponds to 14 days before merger. This only reduces the SNR from 21.9 to 21.3.

![Sky localization for a GW170817-like injection with the LGWA. ](figures/ra_dec_corner_gw170817.png){#fig-lgwa-gw170817-sky width=60%}

For GW250114, despite the higher SNR, the localization is much less precise.
We can see two nearby modes in the sky, corresponding respectively to a face-on source with $\theta_{JN} < \pi / 2$ and a face-off source with $\theta_{JN}> \pi / 2$.
...
![Sky localization and inclination angle for a GW250114-like injection. We compare the results with those obtained with other detectors: the Einstein Telescope in its 2L configuration, and the posterior distribution provided by the LVK collaboration for the real event.](figures/ra_dec_theta_jn_lgwa_vs_et.pdf){#fig-lgwa-gw250114-sky width=80%}

#### Intrinsic parameter constraints
...
The LGWA measurement of chirp mass is extremely precise, with a 90% width of $0.0002M_{\odot}$; for reference, the interval for the Einstein Telescope is roughly 20 times wider, and that in the original LVK analysis is 4000 times wider, at roughly $0.8M_{\odot}$.
...
The Einstein Telescope exhibits extremely precise posterior distributions for both.

![Posterior distribution for the chirp mass, mass ratio and effective spin parameters of our GW250114-like injection.](figures/mchirp_q_lgwa_vs_et.pdf){#fig-lgwa-gw250114-mqchi width=80%}
...
We expect this to mean it will be very effective at constraining other effects, such as contributions from dark matter in the environment, or eccentricity, both of which modify the frequency evolution.


## Reference frames for long observations {#sec-lgwa-reference-frames}

In @sec-timing-general we introduced the problem of choosing a reference frame to perform the analysis. LGWA observations of stellar mass binaries are generally at least a month long, therefore the curvature of the lunar orbit around the Sun is non-negligible: a frame centered on the Earth or Moon would be measurably accelerated compared to the source.
...
We start from the assumption of having a completed parameter estimation run, for which we measured merger time referring to some center $r_{0}$. 
...
The timing uncertainty is a function of four variables ($\vec{r}$, $f$) which we can visualize this in a few different ways, using the injection with GW250114 described in @sec-lgwa-pe-stellar as our reference example.

![Timing uncertainty by frequency for our GW250114 injection.](figures/timing_constraint_by_frequency_GW250114.pdf){#fig-timing-lgwa-by-frequency width=80%}
...
One can see a minimum around 0.54Hz, just above the point where the LGWA's sensitivity is best.

![Spatial variation of the timing uncertainty for our GW250114 injection.](figures/time_std_solar_system.pdf){#fig-timing-lgwa-solar-system width=80%}
...
We also fix three orthogonal spatial slices, each passing through the global minimum, and compute the timing uncertainty in these slices, so as to show its 3D structure. 
We align the $z$ axis in this frame with the injected source direction. 
The global minimum in @fig-timing-lgwa-by-frequency represents a curve in this visualization, $\vec{r}_\text{min}(f)$. We show its projection onto each of the three planes in red, and similarly the trajectories of the Earth and Moon in green and grey. 
...
Then, we assume that our time parameter also $t$ has zero mean. 
The parameter $\gamma$ is always determined if $\alpha$ and $\beta$ are given by normalization, and for a tight localization $|\gamma| \ll |\alpha|, |\beta|$.  Nevertheless, we will include $\gamma$ in our calculations. 
Therefore, we only write down the covariance matrix for $t$, $\alpha$ and $\beta$.
We assume to be able to find a location such that this is diagonal:
$$
\Sigma = \begin{pmatrix}
\sigma_{t}^{2} & 0 & 0 \\
0 & \sigma_{\alpha}^{2} & 0 \\
...
Then, the question is posed as follows: what happens to the timing variance under the transformation
$$
...
Neglecting the terms depending on $\gamma$, which is subdominant for a relatively tight localization, we can recognize the ellipses from @fig-timing-lgwa-solar-system.

:::

![Velocity of the minimum uncertainty position](figures/minimum_uncertainty_velocity.pdf){#fig-minimum-uncertainty-velocity width=80%}

Finally, in @fig-minimum-uncertainty-velocity we show the velocity of the point $\vec{r}_\text{min} (t(f))$, where we compute the time associated with frequency $f$ based on a SPA of the injected signal. 
...
Fisher results (@eq-wen-chen-area) suggest that the sky localization obtained with a long observation scales with the inverse of the area, all else being equal.

In order to validate this, we set up 12 injections with the same parameters as our baseline GW250114 analysis, except for the following:
...
![Waveforms for the 11 and 12 month normalized injections.](figures/lgwa_waveforms_month_by_month.pdf){#fig-waveforms-lgwa-injections width=80%}

According to @eq-wen-chen-area the sky localization area should scale inversely with $A_{\perp}$, which we compute by projecting the detector's orbit onto the plane orthogonal to the injected source's direction, and then taking its convex hull, as shown in @fig-trajectories-lgwa-injections.

![Convex hulls for the 12 month normalized injections.](figures/trajectories.pdf){#fig-trajectories-lgwa-injections width=80%}
...
The shapes of the localization areas are shown in @fig-injections-sky-areas, while @fig-injections-vs-wen-chen summarizes the comparison between these setups and the Fisher expression.

![Sky localization areas for the 12 month normalized injections.](figures/sky_locs.pdf){#fig-injections-sky-areas width=90%}

![Comparison between the Fisher expectation for the sky localization and the injections performed in the three scenarios described in @sec-area-validation.](figures/sky_loc_vs_theory.pdf){#fig-injections-vs-wen-chen width=60%}
...
As a general pattern, we see how the qualitative Fisher prediction is validated for all but the 12 month case: the sky localization area decreases with the number of months, even though the SNR is kept constant, driven mostly by the change in area; the frequency increases slightly through the injections, but not nearly enough to explain the effect.

The failure of the prediction in the 12 month case, with the localization worsening again, is due to the non-uniform accumulation of SNR, with most of it coming from the last few days of observation. 
We validate this intuition through the __simplified, uniform SNR__ scenario, in which the SNR is accumulated at a constant rate over time - we obtain this by artificially setting a PSD in the form $S_{n}(f) \propto f^{4/3}$. 

::: {#nte-uniform-snr-over-time .callout-note collapse="true"}

##### How to obtain uniform SNR over time

For the purpose of validating our intuition, we want to construct a PSD such that the accumulation of SNR over time is uniform, i.e. the SNR obtained in any given month is equivalent to another.  
The $22$ mode of the signal has an amplitude scaling with $f^{-7/6}$;
projecting the signal introduces month-long modulation, which we average over by only looking at integer numbers of months. 
The SNR integrand in time, which we want to be constant, reads as follows when setting an arbitrary power-law PSD with $S_{n}(f) \propto f^{\kappa}$:
$$
\frac{|h(f)|^2}{S_n(f)} \text{d}{f} \propto \frac{f^{-14/6}}{f^{\kappa}} \frac{\text{d}{f}}{\text{d}{t}} \text{d}{t}\,,
$$
and we can obtain the next term by differentiating $t(f) \propto f^{-8/3}$. 
This leads us to $\kappa=4 / 3$.

:::

In all cases, the Fisher approximation overestimates the sky sensitivity of our detector; however, it should be stressed that even when accounting for this effect, the LGWA can still achieve better sky localization than pairs of ground-based interferometers, despite lower SNRs.
...
This points to light sources, with component masses $M \lesssim 30 M_{\odot}$. 
Heavier binaries will have high SNRs, and their intrinsic parameters will be well-constrained, but their localization will not be very precise.

Another intuition we can get from this analysis is that that the hour-long data gaps the LGWA will likely experience, though not yet included in our simulations (see @sec-lgwa-pe-limitations), will not significantly impact sky localization, as long as we can maintain phase coherence across them.
This is because the convex hull of the observation is not significantly affected by them, as on those scales the detector's motion is nearly linear.