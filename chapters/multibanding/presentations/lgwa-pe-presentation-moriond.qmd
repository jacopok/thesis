...
ax = mismatch_plot(
    params, 
    np.geomspace(fmin, fmax, num=500), 
    n_grid=n_grid, 
...
x = np.linspace(-6e-7, 6e-7, num=500)
y = np.linspace(0.86, 0.94, num=500)

# plot_contours(post_ew['chirp_mass']-mc, post_ew['mass_ratio'], ax=plt.gca(), levels=[.9], cmap=cmap_ew, x=x, y=y, N=500)
plot_contours(
    post_full['chirp_mass']-mc, 
    post_full['mass_ratio'], ax=plt.gca(), levels=[.5, .9], cmap=cmap_full, x=x, y=y, N=500)
plt.grid(which='both')
plt.xlim(-2e-7, 2e-7)
...
plot_contours(post_full['ra'], post_full['dec'], ax=plt.gca(), levels=[.5, .9], cmap=cmap_full, N=500)
