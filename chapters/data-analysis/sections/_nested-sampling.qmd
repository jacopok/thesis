...
suppose we have a set of data points $(x_{i}, y_{i})$ with constant uncertainties $\sigma_{y}$ on the $y$ values.
...
Markdown(f'The log Bayes factor is {bayes_factor:.2f}$\pm$ {bf_error:.2f} nats in favor of the parabolic model.')
```
...
Markdown(f'The results are compatible: the value of $H$ obtained from sampling is {sampler.results.information[-1]:.2f}, while the'
...
Thus, we can find a termination condition based on diagnosing whether this is happening: for example, we can compute the contribution to the evidence integral from the current live points, and stop iterating when that becomes smaller than a given threshold --- this is the default in `dynesty` [@speagleDYNESTYDynamicNested2020].
