...
for example, one could model it with a normalizing flow (@sec-normalizing-flows), or if its density function is known one could perform the analysis with an approximate analytic prior and then reweight the samples as described in @sec-weighted-samples.
These approaches are, however, subject to potential pitfalls (failed convergence, low sample efficiency, dependence on hyperparameters), and their performance needs to be studied on a case-by-case basis.
...
if not (data_path / 'cache').exists():
	import os
	os.mkdir((data_path / 'cache').as_posix())
