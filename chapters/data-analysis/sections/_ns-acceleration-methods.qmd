...
A useful observation [@chenBayesianPosteriorRepartitioning2022, @petrosyanSuperNestAcceleratedNested2022], then, is that when studying an inference problem defined by a likelihood $\mathcal{L}$ and prior $\pi$ we can switch to a different one, defined by $\mathcal{\tilde{L}}$ and $\tilde{\pi}$: as long as at every point
$$
\tilde{\mathcal{L}}(\theta) \tilde{\pi}(\theta) = \mathcal{L} (\theta)\pi(\theta)
$$
the evidences, as well as the posteriors, will be the same.
