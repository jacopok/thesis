...
The Fourier transform in equation @eq-fourier-transform-discrete is, technically speaking, only valid if the data is _periodic_. 
This is not the case for realistic data: on the contrary, we only want to consider a given segment in time around our event.
...
![_p_-_p_ plot for the distance-only injections with and without the correction. From @talbotInferenceFiniteTime2025.](figures/pp-test.pdf)

  

![K-S test _p_-value as a function of the number of injected signals, from @talbotInferenceFiniteTime2025.](figures/pvalue_logit.png)

:::

It shows signs of underconstraining, see @fig-pp-plot-bias-underconstrain.
TODO it should be overconstrained! 
...
## Matched filtering and the signal-to-noise ratio
