...
Sometimes, the _Amplitude Spectral Density_ (ASD) is also quoted, which is the square root of the PSD, with units of $\text{Hz}^{-1/2}$:^[More in general, the ASD has units of $x / \sqrt{ \text{Hz} }$, where $x$ is the unit of the time-domain noise. For example, if we were characterizing acceleration noise $x$ would be $\text{m/s}^2$.]
$$
A_{n}(f) = \sqrt{ S_{n}(f) }\,.
